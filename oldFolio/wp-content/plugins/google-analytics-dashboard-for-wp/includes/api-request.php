	...
	public function request() {
		...
		if ( 200 != $response_code ) {
			...
			if ( empty( $response_body ) || ( empty( $response_body['message'] ) && empty( $response_body['error'] ) ) ) {
				...
				$message = sprintf(
					esc_html__( 'Oops! We ran into a problem. Please try again in a few minutes. If the issue persists please %1$scontact our support%2$s team. Error: API returned a <strong>%3$s</strong> response.', 'google-analytics-dashboard-for-wp' ),
					'<a target="_blank" href="' . exactmetrics_get_url( 'notice', 'unknown-api-error', 'https://www.exactmetrics.com/my-account/support/' ) . '">',
					'</a>',
					$response_code
				);
			...
			if ( ! empty( $response_body['message'] ) ) {
				...
				$message = sprintf(
					esc_html__( 'Oops! We ran into a problem. Please try again in a few minutes. If the issue persists please %1$scontact our support%2$s team. Error: API returned a <strong>%3$d: %4$s</strong>.', 'google-analytics-dashboard-for-wp' ),
					'<a target="_blank" href="' . exactmetrics_get_url( 'notice', 'unknown-api-error', 'https://www.exactmetrics.com/my-account/support/' ) . '">',
					'</a>',
					$response_code,
