...
class Moesia_Projects_Filtered extends WP_Widget {

// constructor
    function moesia_projects_filtered() {
		$widget_ops = array('classname' => 'moesia_projects_filtered_widget', 'description' => __( 'Show your most intresting projects.', 'moesia') );
        parent::WP_Widget(false, $name = __('Moesia FP: Filtered Projects', 'moesia'), $widget_ops);
		$this->alt_option_name = 'moesia_projects_filtered_widget';
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['moesia_projects_filtered']) )
			delete_option('moesia_projects_filtered');		  
		  
		return $instance;
	}
	
	function flush_widget_cache() {
		wp_cache_delete('moesia_projects_filtered', 'widget');
	}
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'moesia_projects_filtered', 'widget' );
		}
...
?>
				...
				<?php
					...
					if ( $count > 0 ){
						...
						foreach ( $terms as $term ) {
							...
							echo '<li><a href="javascript:void(0)" title="" data-filter=".'.$termname.'">'.$term->name.'</a><span class="seperator">/</span></li>';
						}
					}
					
					echo '<li><a href="javascript:void(0)" title="" data-filter=".all">View All</a></li>';
				?>
			</ul>
			<div id="isotope-list" class="container">
				...
				<?php while ( $r->have_posts() ) : $r->the_post(); ?>
					...
					<?php $terms = get_the_terms( $post->ID, 'portfolio-categories' );	
					...
					?>
					<div class="project col-md-4 col-sm-6 col-xs-6 all <?php echo $tax; ?>">
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'moesia_projects_filtered', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
