...
class Moesia_Projects_Custom extends WP_Widget {

// constructor
    function moesia_projects_custom() {
		$widget_ops = array('classname' => 'moesia_projects_custom_widget', 'description' => __( 'Show your most intresting projects.', 'moesia') );
        parent::__construct(false, $name = __('Moesia FP: Projects', 'moesia'), $widget_ops);
		$this->alt_option_name = 'moesia_projects_custom_widget';
    }
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['moesia_projects_custom']) )
			delete_option('moesia_projects_custom');		  
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'moesia_projects_custom', 'widget' );
		}
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'moesia_projects_custom', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
